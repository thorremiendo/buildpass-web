...
export class EvaluatorPersonalInfoComponent implements OnInit {
  ...
  ngOnInit(): void {
    this._barangayService.getBarangayInfo().subscribe((data) => {
      this.barangay = data;
      this._filteredBarangayOptions = this.evaluatorPersonalInfoFormControl.barangay.valueChanges.pipe(
        startWith(''),
        map((barangay) =>
          barangay ? this.filter(barangay) : this.barangay.slice()
        )
      );
    });

    this._registerAccountEvaluatorFormService.cast.subscribe(
      (registerAccountEvaluatorSubject) =>
        (this.userDetails = registerAccountEvaluatorSubject)
    );

    if (this.userDetails && this.userDetails.firebase_uid) {
      this.persist(
        'evaluator-registration-firebase-uid',
        this.userDetails.firebase_uid
      );
      this.persist(
        'evaluator-registration-email-address',
        this.userDetails.email_address
      );
      this.persist(
        'evaluator-registration-email-verified',
        this.userDetails.emailVerified
      );
      this.persist(
        'evaluator-registration-is-evaluator',
        this.userDetails.is_evaluator
      );
      this.persist(
  ...
  createForm() {
    const firstName = this._localStorageService.get(
      'evaluator-registration-first-name'
    );
    const middleName = this._localStorageService.get(
      'evaluator-registration-middle-name'
    );
    const lastName = this._localStorageService.get(
      'evaluator-registration-last-name'
    );
    const suffixName = this._localStorageService.get(
      'evaluator-registration-suffix-name'
    );
    const birthDate = this._localStorageService.get(
      'evaluator-registration-birth-date'
    );
    const gender = this._localStorageService.get(
      'evaluator-registration-gender'
    );
    const maritalStatus = this._localStorageService.get(
      'evaluator-registration-marital-status'
    );
    const homeAddress = this._localStorageService.get(
      'evaluator-registration-home-address'
    );
    const barangay = this._localStorageService.get(
